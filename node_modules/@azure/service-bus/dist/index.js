...
class ServiceBusAdministrationClient extends coreClient.ServiceClient {
    ...
    options3) {
        ...
        const serviceClientOptions = coreRestPipeline.createPipelineFromOptions(Object.assign(Object.assign({}, options), { userAgentOptions: {
                userAgentPrefix,
            } }));
...
function toRheaMessage(msg, encoder) {
    ...
    if (amqpMsg.ttl != null && amqpMsg.ttl !== coreAmqp.Constants.maxDurationValue) {
        ...
        amqpMsg.absolute_expiry_time = new Date(Math.min(coreAmqp.Constants.maxAbsoluteExpiryTime, amqpMsg.creation_time.getTime() + amqpMsg.ttl));
    }
    if (isAmqpAnnotatedMessage(msg)) {
        return amqpMsg;
    }
...
class ServiceBusMessageImpl {
    ...
    constructor(msg, delivery, shouldReorderLockToken, receiveMode, skipParsingBodyAsJson, skipConvertingDate) {
        const _a = fromRheaMessage(msg, { skipParsingBodyAsJson, delivery, shouldReorderLockToken, skipConvertingDate }), { _rawAmqpMessage } = _a, restOfMessageProps = tslib.__rest(_a, ["_rawAmqpMessage"]);
...
class ManagementClient extends LinkEntity {
    ...
    constructor(context, entityPath, options) {
         ...
         */
        this._initLock = rheaPromise.generate_uuid();
        this._context = context;
    }
    /**
    ...
    async initWithUniqueReplyTo(options = {}) {
        var _a;
        const retryTimeoutInMs = (_a = options.timeoutInMs) !== null && _a !== void 0 ? _a : coreAmqp.Constants.defaultOperationTimeoutInMs;
        ...
        return coreAmqp.defaultCancellableLock.acquire(this._initLock, async () => {
            ...
            const { abortSignal } = options !== null && options !== void 0 ? options : {};
        ...
        try {
            ...
            const request = {
                body: {
                    ...
                    skip: (updatedOptions === null || updatedOptions === void 0 ? void 0 : updatedOptions.skip) ? rheaPromise.types.wrap_int(updatedOptions.skip) : rheaPromise.types.wrap_int(0),
                },
                reply_to: this.replyTo,
                application_properties: {
...
function toProcessingSpanOptions(receivedMessages, receiver, connectionConfig) {
    ...
    return Object.assign({ spanLinks, spanKind: "consumer" }, toSpanOptions({ host: connectionConfig.host, entityPath: receiver.entityPath }));
}
...
function completeMessage(message, context, entityPath, retryOptions) {
    ...
    return tracingClient.withSpan("ServicebusReceiver.complete", {}, () => settleMessage(message, DispositionType.complete, context, entityPath, {
        retryOptions,
    }), Object.assign({ spanLinks }, toSpanOptions({ entityPath, host: context.config.host }, "client")));
}
/**
 * @internal
 *
...
function deadLetterMessage(message, context, entityPath, propertiesToModify, retryOptions) {
    ...
    const tracingContext = extractSpanContextFromServiceBusMessage(message);
    const spanLinks = tracingContext ? [{ tracingContext }] : [];
    return tracingClient.withSpan("ServiceBusReceiver.deadLetter", {}, () => settleMessage(message, DispositionType.deadletter, context, entityPath, dispositionStatusOptions), Object.assign({ spanLinks }, toSpanOptions({ entityPath, host: context.config.host }, "client")));
}
/**
 * @internal
 */
    ...
    _setMessageHandlers(userHandlers, operationOptions) {
        ...
        const messageHandlers = {
            ...
            processMessage: async (message) => {
                try {
                    await tracingClient.withSpan("StreamReceiver.process", operationOptions !== null && operationOptions !== void 0 ? operationOptions : {}, () => userHandlers.processMessage(message), toProcessingSpanOptions(message, this, this._context.config));
                }
                catch (err) {
                    this._messageHandlers().processError({
                        error: err,
...
class BatchingReceiverLite {
    ...
    async receiveMessages(args) {
        try {
            ...
            return tracingClient.withSpan("BatchingReceiverLite.process", args, () => messages, toProcessingSpanOptions(messages, this, this._connectionContext.config));
        }
        finally {
    ...
    async tryDrainReceiver(receiver, loggingPrefix, abortSignal) {
        ...
        receiver.drainCredit();
        receiverLogger.verbose(`${loggingPrefix} Draining leftover credits(${receiver.credit}), waiting for event_drained event, or timing out after ${this._drainTimeoutInMs} milliseconds...`);
        await drainPromise;
        if (drainTimedout) {
            receiverLogger.warning(`${loggingPrefix} Time out after ${this._drainTimeoutInMs} milliseconds when draining credits. Closing receiver...`);
...
class ServiceBusReceiverImpl {
    ...
    async renewMessageLock(message) {
        ...
        }, Object.assign({ spanLinks }, toSpanOptions({ entityPath: this.entityPath, host: this._context.config.host }, "client")));
    }
    async close() {
        try {
            this._isClosed = true;
...
class ServiceBusSessionReceiverImpl {
    ...
    subscribe(handlers, options) {
        ...
        this._registerMessageHandler(async (message) => {
            return tracingClient.withSpan("SessionReceiver.process", options !== null && options !== void 0 ? options : {}, () => handlers.processMessage(message), toProcessingSpanOptions(message, this, this._context.config));
...
class ServiceBusSenderImpl {
    ...
    async sendMessages(messages, options) {
        ...
        if (!isServiceBusMessageBatch(messages) && !Array.isArray(messages)) {
            ...
            const { message, spanContext } = instrumentMessage(originalMessage, options !== null && options !== void 0 ? options : {}, this.entityPath, this._context.config.host);
            const spanLinks = spanContext ? [{ tracingContext: spanContext }] : [];
            return tracingClient.withSpan("ServiceBusSender.send", options !== null && options !== void 0 ? options : {}, (updatedOptions) => this._sender.send(message, updatedOptions), Object.assign({ spanLinks }, toSpanOptions({ entityPath: this.entityPath, host: this._context.config.host }, "client")));
        }
        let batch;
        ...
        return tracingClient.withSpan("ServiceBusSender.send", options !== null && options !== void 0 ? options : {}, (updatedOptions) => this._sender.sendBatch(batch, updatedOptions), Object.assign({ spanLinks }, toSpanOptions({ entityPath: this.entityPath, host: this._context.config.host }, "client")));
    }
...
class MessageSession extends LinkEntity {
    ...
    static async create(identifier, context, entityPath, sessionId, options) {
        ...
        if (sessionId === undefined) {
            timeoutInMs = (_b = (_a = options.retryOptions) === null || _a === void 0 ? void 0 : _a.timeoutInMs) !== null && _b !== void 0 ? _b : coreAmqp.Constants.defaultOperationTimeoutInMs;
