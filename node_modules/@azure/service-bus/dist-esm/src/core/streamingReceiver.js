    ...
    constructor(identifier, connectionContext, entityPath, options) {
        ...
        this._onSessionClose = async (context) => {
            ...
            if (receiver && !receiver.isSessionItselfClosed()) {
                ...
                logger.verbose("%s 'session_close' event occurred on the session of receiver '%s' with address " +
        ...
        this._onSessionError = (context) => {
            ...
            if (sessionError) {
                ...
                logger.logError(sbError, `${this.logPrefix} 'session_error' event occurred. The associated error is`);
                this._messageHandlers().processError({
                    error: sbError,
                    errorSource: "receive",
                    entityPath: this.entityPath,
        ...
        this._onAmqpMessage = async (context) => {
            ...
            try {
                ...
                if (!bMessage.delivery.remote_settled &&
                    ...
                    try {
                        logger.logError(error, "%s Abandoning the message with id '%s' on the receiver '%s' since " +
    ...
    async onDetached(receiverError) {
        try {
            ...
            if (this.wasClosedPermanently) {
                logger.verbose(`${this.logPrefix} onDetached: link has been closed permanently, not reinitializing. `);
                return;
            }
            ...
            if (this._isDetaching) {
                ...
                logger.verbose(`${this.logPrefix} onDetached: Call to detached on streaming receiver '${this.name}' is already in progress.`);
                return;
            }
            ...
            const translatedError = receiverError
                ...
                : receiverError;
            logger.logError(translatedError, `${this.logPrefix} onDetached: Reinitializing receiver because of error`);
            // Clears the token renewal timer. Closes the link and its session if they are open.
            // Removes the link and its session if they are present in rhea's cache.
            await this.closeLink();
        }
        catch (err) {
            logger.verbose(`${this.logPrefix} onDetached: Encountered an error when closing the previous link: `, err);
        }
        try {
