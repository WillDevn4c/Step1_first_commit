...
export class BatchingReceiverLite {
    ...
    async tryDrainReceiver(receiver, loggingPrefix, abortSignal) {
        ...
        logger.verbose(`${loggingPrefix} Draining leftover credits(${receiver.credit}), waiting for event_drained event, or timing out after ${this._drainTimeoutInMs} milliseconds...`);
        await drainPromise;
        if (drainTimedout) {
            logger.warning(`${loggingPrefix} Time out after ${this._drainTimeoutInMs} milliseconds when draining credits. Closing receiver...`);
